...
const CHAPTER_SECTION = {
  语文: {
    现代文阅读: '现代文阅读', 小说阅读: '现代文阅读', 散文阅读: '现代文阅读', 实用类文本阅读: '现代文阅读',
    文言文阅读: '文言文阅读', 古代诗文阅读: '文言文阅读',
    名句名篇默写: '名句名篇默写', 语言文字运用: '语言文字运用',
    作文: '写作', 写作素材积累与运用: '写作',
  },
  数学: {},
  英语: {
...
// 无章节映射时的兜底：按题型轮转分区
const TYPE_SECTIONS = {
  语文: { choice: ['现代文阅读', '文言文阅读', '古代诗歌鉴赏', '语言文字运用'], fill: ['名句名篇默写'], essay: ['写作'] },
  数学: { choice: ['单选题'], fill: ['填空题'], essay: ['解答题'] },
  英语: { choice: ['单项选择', '完形填空', '阅读理解'], fill: ['语法填空'], essay: ['书面表达'] },
};

// 真实结构模板（广东春季高考·依学考，满分150）
const TEMPLATES = {
  语文: {
    name: '语文·春季高考仿真卷',
    description: '按广东春季高考（依学考）真实题型组卷：现代文阅读/文言文阅读/古代诗歌鉴赏/名句默写/语言运用/写作，满分150分，120分钟',
    totalScore: 150, duration: 120,
    sections: [
      { name: '现代文阅读', type: 'choice', count: 8, scorePer: 4 },
      { name: '文言文阅读', type: 'choice', count: 6, scorePer: 3 },
      { name: '古代诗歌鉴赏', type: 'choice', count: 4, scorePer: 3 },
      { name: '名句名篇默写', type: 'fill', count: 6, scorePer: 2 },
      { name: '语言文字运用', type: 'choice', count: 4, scorePer: 4 },
...
async function tagQuestions() {
  ...
  for (const s of subjects) {
    ...
    for (const q of questions) {
      ...
      if (!section && qkFallback(q, typeFallback, roundRobin)) {
        section = qkFallback(q, typeFallback, roundRobin);
      }
      if (section) {
...
async function updateTemplates() {
  ...
  for (const s of subjects) {
    ...
    if (existing) {
      ...
      console.log(`📄 ${s.name}: 已更新模板「${tpl.name}」(150分/${tpl.duration}分钟)`);
    } else {
